...
lemaCombo1 <- lemaCombo[lemaCombo$naCount < 2,] 

# get the weather/soil code for each cell
lemaCombo1$static <- apply(lemaCombo1, 1, function(x) { 
...
someNAs <- lemaCombo[!complete.cases(lemaCombo),] 
someNAs$naCount <- apply(someNAs, 1, function(x) sum(is.na(x)))
lessNAs <- someNAs %>% filter(naCount < 6)
...
write.csv(gridExps, row.names=FALSE,
          file = paste0(filterDir, '/uniqueExperiments_gmd4p_Sheridan6_gridcellKey.csv'))
# save key and unique combinations as RData
save(lemaExps,gridExps, 
     file = paste0(filterDir,'/uniqueExperiments_gmd4p_Sheridan6.RData'))
```
...
spplot(outRasClip, add=T, col.regions='blue', maxPixels = 500000,
       main = 'Active Cells')
```
