...
Goal: Retrieve crop management parameters from the NASS Quickstats API. While we're at it go ahead and get yields.
...
* Data Item: ' - PROGRESS, MEASURED IN PCT PLANTED' ...etc, pasted to crop type but not exactly
* Domain: 'TOTAL'
...
getPlantingDate <- function(apikey, commodity, category, shortDesc, state, year){
  # build URL query
...
for(i in 1:nrow(cropsWanted)){
  dates.list[[i]] <- getPlantingDate(apiKey, cropsWanted[i,'crop'], category,
...
for(i in 1:nrow(cropsWanted)){
  width.list[[i]] <- getPlantingDate(apiKey, cropsWanted2[i,'crop'], category,
...
density.df <- getPlantingDate(apiKey, crop, category, short, state, year)
