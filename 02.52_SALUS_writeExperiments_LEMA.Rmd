...
cropParamKey <- data.frame(crop = c('CORN','SOYBEANS','WHEAT','SORGHUM','GRASS',
                           ...
                           Ppop_rain = c('4', '39.54', '494.21', '8.6', '', ''),
                           Ppop_irr = c('7.5', '39.54', '494.21', '8.6', '', ''),
                           RowSpc = c('76', '54', '19.05','50.8', '', ''),
...
expt_set <- lemaExps2 %>% 
  filter(static == '2011499941122') %>%
  mutate(xdbCode = paste0('LX_','1'))
...
testOutDir <- 'C:/Users/deinesji/Documents/Rprojects/lema_SALUS_experiment_files/test_lema_realLife'
# Write the file! --------------------------------------------------------------


outFile2 <- paste0(testOutDir,'/',expt_set$xdbCode[1], '.xdb.xml')
...
for(i in 1:nrow(exp_master)){  
  ...
  annualSpecs <- rotations %>% left_join(cropManagement)
  
  # write rotation and management parameters for each year
  for(m in 1:nrow(annualSpecs)){
    # set irrigation flags
    if (annualSpecs$irrStatus[m] == 1) irrig <- 'F'
    if (annualSpecs$irrStatus[m] == 0) irrig <- 'N'
