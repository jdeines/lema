...
```{r fillMissing}
# get files
files <- list.files(xmlDir, full.names=TRUE)

# process each in a loop
for(m in 1:length(files)){
  # load file and extract df
  xml <- read_xml(files[m])
  xml_children(xml)
  blah <- xml_find_first(xml,"Stations")
  xml_children(blah)
  weather <- xml_find_first(blah,"Weather")
  weatherAttr <- xml_attrs(weather)
  weatherList <- as_list(weather)
  
  library(XML)
  test <- xmlParse(files[m])  
  testList <- xmlToList(test)
  stationList <- testList[['Stations']]
  weather <- stationList[['Weather']]
  wdf <- xmlToDataFrame(weather)
}
