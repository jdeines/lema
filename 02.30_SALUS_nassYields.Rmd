...
crop | pounds/bushel | kg/ha = x bu/acre
--------------- | ------ | --------
corn | 56 | 0.0159
soy/wheat | 60 | 0.0149
sorghum, grain | 56 | 0.0159
...
yield.clean <- yield.raw %>%
  ...
  rename(crop = commodity_desc, yield = Value) %>%
  # drop most columns
  select(c(year, fips5, county_name, crop, type, yield, unit_desc, state_name)) %>%
...
Overall regional yields by crop type and irrigation status:

```{r regionalYields}
# format data
regional <- yield.clean %>%
  group_by(year, crop, type) %>%
  do(data.frame(t(quantile(.$yield, probs = c(.1, .25, .5, .75, .9),na.rm=TRUE))))

# plot
ggplot(regional, aes(x=year, y = X50., color = type)) +
...
state.clean <- state.raw %>%
  ...
  rename(crop = commodity_desc, yield = Value) %>%
  # drop most columns
  select(c(year, crop, type, yield, unit_desc, state_name)) %>%
