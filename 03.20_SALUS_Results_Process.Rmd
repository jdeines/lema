...
var.annual2 <- var.annual %>% left_join(gridExps, by = 'ExpID')

# join results to template vector
template.df <- data.frame(gridcell = templateVector)
irrVectors <-  template.df %>%
  left_join(var.annual2, by = 'gridcell')

# rasterize data vector
outRas <- template
outRas[] <- irrVectors[,'x2006']

outRasClip <- crop(outRas, aoi)
spplot(outRasClip, add=T, col.regions=bpy.colors(20), maxPixels = 500000,
       main = 'Irrigation Applied (mm), 2006')
```
