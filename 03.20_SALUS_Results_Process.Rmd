...
```{r spIrrigation, fig.width = 9, fig.height = 5}
# load formatted results output
expt.annual <- read_csv(paste0(resultsDir, '/', runname, '.csv'))
variable <- 'IRRC_mm'
...
for(year in yearsWanted){
  ...
  irrList[[rasName]] <- salustools::spatialize(expt.annual, variable, year, 
                                        gridExps, template,aoi)
}  
irrStack <- stack(irrList, quick = TRUE)

spplot(irrStack, main = 'SALUS Irrigation (mm), run 3', maxpixels = 200000)
```

A few individual year maps, and get total irrigation water applied time series. Includes converting mm to m2
```{r spIrr2}
...
for(year in yearsWanted){
  rasName <- as.character(year)
  pcpList[[rasName]] <- salustools::spatialize(expt.annual, variable, year, 
                                        gridExps, template,aoi)
}  
pcpStack <- stack(pcpList, quick = TRUE)

spplot(pcpStack, main = 'NLDAS Precip (mm), run 3', maxpixels = 200000)
```
...
for(year in yearsWanted){
  rasName <- as.character(year)
  rchList[[rasName]] <- salustools::spatialize(expt.annual, variable, year, 
                                        gridExps, template,aoi)
}  
rchStack <- stack(rchList, quick = TRUE)

spplot(rchStack, main = 'SALUS Recharge (mm), run 3', maxpixels = 200000)
```
